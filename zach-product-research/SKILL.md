...
**免费 Playwright 模式专用脚本（无需 Sorftime 账号）：**

- `scripts/bsr_estimator.py`
  - 用途：根据 BSR 幂律模型估算月销量（`base_rate × BSR^-0.55`）
  - 命令（批量）：`python scripts/bsr_estimator.py --batch products_merged.json --out products_estimated.json`
  - 命令（单品）：`python scripts/bsr_estimator.py --bsr 5000 --category "Home & Kitchen"`
- `scripts/analyze_products.py`
  - 用途：对 Playwright 抓取的产品集打竞争分数（1-10）+ 机会分数（1-10），生成 signals / red_flags
  - 命令：`python scripts/analyze_products.py --input products_estimated.json --output analysis.json`
- `scripts/render_report.py`
  - 用途：将 analysis.json + products JSON 渲染为 Markdown 报告
  - 命令：`python scripts/render_report.py --products products_estimated.json --analysis analysis.json --keyword "关键词" --out report.md`

## References

- `references/category_benchmarks.json`
  - 用途：21 个 Amazon 一级类目的 BSR 基准销量（免费 Playwright 模式专用）
  - 格式：`{ "Home & Kitchen": { "base_rate": 5000, "decay_exponent": 0.55 }, ... }`
...
## 免费 Playwright 模式（无 Sorftime 账号时使用）

> **触发条件**：用户无 Sorftime 账号，或 Sorftime MCP 不可用时，自动切换到本模式。
> 数据来源：Amazon 页面 Playwright 实时抓取 + BSR 幂律销量估算。
> 精度低于 Sorftime 模式，但完全免费，适合快速验证机会。

### 模式对比

| 维度 | Sorftime MCP 模式 | 免费 Playwright 模式 |
|------|-------------------|----------------------|
| 数据精度 | 高（官方历史数据）| 中（页面快照 + BSR 估算）|
| BSR 可获取 | ✅ 有 | ⚠️ 部分有（需产品详情页）|
| 月销量 | 精确 | 估算值（幂律模型）|
| 差评数据 | `product_reviews` 工具 | 抓取评论页 |
| Keepa 价格历史 | 内置 | Playwright 截图 |
| 费用 | Sorftime 订阅 | 免费 |
| 适用场景 | 正式选品决策 | 快速机会扫描、轻量验证 |
...
确认以下信息：
1. **关键词**：如 `LED neon sign bedroom`
2. **站点**：默认 `amazon.com`
3. **输出目录**：默认 `C:\Users\Administrator\Desktop\research\{keyword-slug}\`

创建输出目录：
```powershell
New-Item -ItemType Directory -Force -Path "C:\Users\Administrator\Desktop\research\{keyword-slug}"
```

---

### 免费模式 Step 1：抓取搜索结果（2 页）

导航到：`https://www.amazon.com/s?k={url-encoded-keyword}`

第 1、2 页各抓一次（`&page=2`），每个产品卡片提取：
- `asin` — `data-asin` 属性
- `title`、`brand`、`price`、`rating`、`review_count`
- `is_sponsored`、`has_bestseller_badge`、`has_amazons_choice`
- `position` — 1-indexed

保存到：`{workdir}\search_raw.json`

**注意**：
- 每页之间等待 2 秒：`mcp__playwright__browser_wait_for`
- 出现 CAPTCHA 立即停止告知用户
- 快照超大时：用 `filename` 参数保存到文件，再用 Explore subagent 解析

---

### 免费模式 Step 2：抓取产品详情页（Top 15）

取前 15 个非赞助 ASIN，逐个导航到 `https://www.amazon.com/dp/{asin}` 提取：

| 字段 | 说明 |
|------|------|
| `title`, `brand`, `price`, `rating`, `review_count` | 基础信息 |
| `bsr` | Best Seller Rank（整数，无则为 0）|
| `bsr_category` | BSR 所属一级类目 |
| `is_amazon_sold`, `is_fba` | 卖家类型 |
| `offer_count`, `variation_count`, `images_count` | 竞争信号 |
| `has_aplus`, `date_first_available` | 运营成熟度 |

保存到：`{workdir}\products_detail.json`

---

### 免费模式 Step 3：抓取 Best Sellers（可选）

找到主类目的 Best Sellers 页面：
`https://www.amazon.com/Best-Sellers/zgbs/{category-path}/`

提取前 20 个：rank, asin, title, price, rating, review_count

保存到：`{workdir}\bestsellers.json`

---

### 免费模式 Step 4：评论情感（按评论数 Top 5 产品）

对评论数最多的 5 个产品，抓取：
`https://www.amazon.com/product-reviews/{asin}?sortBy=recent&pageNumber=1`（第 1、2 页，共 20 条/产品）

从评论中识别：
- **Top 3 好评特征**：love / perfect / great / works well / easy
- **Top 3 差评痛点**：broke / cheap / disappointed / returned / doesn't work
- **改进需求**：wish / should / if only / would be better

保存原始数据到：`{workdir}\reviews_raw.json`
保存摘要到：`{workdir}\review_sentiment.json`
...
### 免费模式 Step 5：Keepa 价格历史（按估算销量 Top 3）

对销量估算最高的 3 个产品，导航到：
`https://keepa.com/#!product/1-{asin}`（US 站用 1，其他见下表）

等待 6 秒图表渲染后截图：`mcp__playwright__browser_take_screenshot`
保存到：`{workdir}\keepa_{asin}.png`
...
若 Keepa 加载失败，跳过并注明「Keepa unavailable」，继续下一步。

---

### 免费模式 Step 6：BSR 销量估算

合并 `search_raw.json` 和 `products_detail.json`（详情页字段优先），保存到：
`{workdir}\products_merged.json`

运行：
```powershell
python "C:\Users\Administrator\.claude\skills\zach-product-research\scripts\bsr_estimator.py" `
  --batch "{workdir}\products_merged.json" `
  --out "{workdir}\products_estimated.json"
```

> BSR=0 的产品 estimated_monthly_sales=0（BSR 不可用）。这会低估市场规模，在报告中注明。
...
```powershell
python "C:\Users\Administrator\.claude\skills\zach-product-research\scripts\analyze_products.py" `
...
| 分项 | 竞争分（高=竞争激烈）| 机会分（高=机会好）|
|------|-------------------|--------------------|
| 平均评论数 | ≥2000→8分, ≥50→3分 | 越少越好 |
| 品牌数量 | ≤2品牌→9分, ≥8品牌→3分 | 越分散越好 |
| FBA 比例 | 比例×10 | — |
| 均价区间 | — | $20-80 最佳=8分 |
| 市场月销额 | — | ≥$200K=8分 |
...
```powershell
python "C:\Users\Administrator\.claude\skills\zach-product-research\scripts\render_report.py" `
...
### 免费模式 Step 9：输出 5 点摘要

报告展示后，给用户输出：

1. **结论**（GO / CONDITIONAL GO / NO-GO）+ 一句原因
2. **市场规模** + **均价**
3. **Top 2 机会信号**
4. **Top 2 风险/红旗**
5. **推荐下一步**（如：打样 5 款测 listing / 转子品类 X / 竞争过激放弃）
...
| 情况 | 处理 |
|------|------|
| Amazon CAPTCHA | 立即停止，告知用户换网络或稍后重试 |
| 产品页 404 | 跳过该 ASIN |
| BSR 未显示 | `bsr: 0`，报告中标注「BSR 不可用」 |
| 价格为区间（变体） | 取最低可见价格 |
| Keepa 加载失败 | 跳过，注明「Keepa unavailable」|
| 评论页返回空 | 跳过该 ASIN 的评论抓取 |
| 快照超 25K tokens | 保存到文件后用 Explore subagent 解析 |
...
**免费 Playwright 模式**完成状态（三选一）：
- **FREE_DONE** — `report.md` 生成并展示，5 点摘要已输出
- **FREE_DONE_BSR_MISSING** — 报告生成，但大部分产品 BSR=0，月销量估算不可靠（已在报告中注明）
- **FREE_CAPTCHA_BLOCKED** — Amazon 显示 CAPTCHA，调研中止，建议用户换 IP 或稍后重试
